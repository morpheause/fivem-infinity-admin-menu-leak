...
ESX.RegisterServerCallback('m3:admin:server:refreshBanlist', function(source, cb)
    local cbbantable = {}
    MySQL.Async.fetchAll('SELECT * FROM m3_banlist', {}, function(results)
    ...
    if havePermission(src, 'wipeplayer') then
        if target ~= nil then
            ...
            for i = 1, #sqltables, 1 do
                ...
                MySQL.Async.execute(sqlquery, {['@'..sqltables[i].identifierparameter] = tIdentifier})
            end
        else
            ...
            for i = 1, #sqltables, 1 do
                ...
                MySQL.Async.execute(sqlquery, {['@'..sqltables[i].identifierparameter] = steamhex})
            end
        end
    else
...
AddEventHandler('m3:admin:server:ban', function(target, reason, time)
    ...
    if havePermission(src, 'ban') then
        ...
        MySQL.Async.execute('INSERT INTO m3_banlist (name, admin, steam, rockstar, xbox, live, discord, ip, reason, time) VALUES (@name, @admin, @steam, @rockstar, @xbox, @live, @discord, @ip, @reason, @time)',
...
AddEventHandler('m3:admin:server:unban', function(banid)
    ...
    if havePermission(src, 'unban') then
        MySQL.Async.fetchAll('SELECT * FROM m3_banlist WHERE id = @id', {['@id'] = banid}, function(result)
            if result[1] ~= nil then
                ...
                MySQL.Async.execute('DELETE FROM m3_banlist WHERE id = @id', {['@id'] = banid})
                refreshBanlist()
            end
        end)
    else
...
AddEventHandler('playerConnecting', function(name, setCallback, deferrals) 
    ...
    MySQL.Async.fetchAll('SELECT * FROM m3_banlist WHERE steam = @steam OR rockstar = @rockstar OR xbox = @xbox OR live = @live OR discord = @discord OR ip = @ip', {
        ['@steam'] = identifier,
        ['@rockstar'] = license,
        ['@xbox'] = xblid,
        ['@live'] = liveid,
        ...
        if results[1] ~= nil or #results > 0 then
            for k = 1, #results, 1 do
                if results[k].time > ostime() then
                    ...
                    MySQL.Async.execute('DELETE FROM m3_banlist WHERE id = @id', {['@id'] = results{k}.id})
                    deferrals.done()
                    return
                end
            end
